...
public:
  ...
  int statx(const char *path, unsigned int flags, unsigned int want, struct ceph_statx *stx);
  ...
  int ll_getattrx(Inode *in, unsigned int flags, unsigned int want,
		  struct ceph_statx *st, int uid = -1, int gid = -1);
