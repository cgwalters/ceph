...
TEST(LibCephFS, Btime) {
  ...
  ASSERT_EQ(ceph_statx(cmount, filename, 0, CEPH_STATX_BASIC_STATS | CEPH_STATX_BTIME, &stx), 0);
...
TEST(LibCephFS, SetBtime) {
  ...
  ASSERT_EQ(ceph_statx(cmount, filename, 0, CEPH_STATX_BTIME, &stx), 0);
...
TEST(LibCephFS, LazyStatx) {
  ...
  struct ceph_statx	stx;
  ASSERT_EQ(ceph_ll_getattrx(cmount2, file2, AT_NO_ATTR_SYNC, CEPH_STATX_CTIME, &stx, getuid(), getgid()), 0);
...
TEST(LibCephFS, ChangeAttr) {
  ...
  struct ceph_statx	stx;
  ASSERT_EQ(ceph_statx(cmount, filename, 0,  CEPH_STATX_VERSION, &stx), 0);
...
TEST(LibCephFS, DirChangeAttr) {
  ...
  struct ceph_statx	stx;
  ASSERT_EQ(ceph_statx(cmount, dirname, 0, CEPH_STATX_VERSION, &stx), 0);
  ASSERT_TRUE(stx.stx_mask & CEPH_STATX_VERSION);
